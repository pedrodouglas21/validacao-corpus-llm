...
function badge(status){return `<span class="status ${status}">${status.replaceAll("_"," ")}</span>`}
...
async function load(){
  ...
  const fully=rows.filter(x=>Number(x.n_ratings)>=CONFIG.REQUIRED_RATERS);
  const scvi=fully.length?fully.reduce((s,r)=>s+Number(r.i_cvi||0),0)/fully.length:null;
  $("mEvaluations").textContent=complete.length;
  $("mReady").textContent=`${fully.length} / ${rows.length}`;
  $("mSCVI").textContent=scvi==null?"—":scvi.toFixed(3);
  $("mReview").textContent=rows.filter(r=>Number(r.n_ratings)>=CONFIG.REQUIRED_RATERS && Number(r.i_cvi)<CONFIG.I_CVI_THRESHOLD).length;

  $("caseTable").innerHTML=rows.map(r=>`<tr>
    <td><strong>${r.case_code}</strong></td>
    <td>${r.proposed_role}${r.backup_rank?` #${r.backup_rank}`:""}</td>
    <td>${r.n_ratings}</td><td>${fmt(r.i_cvi)}</td>
    ...
    <td>${r.blocking_flags}</td><td>${badge(r.status)}</td></tr>`).join("");
  ...
  $("evaluatorGrid").innerHTML=(profiles||[]).map((p,i)=>`<div class="evaluator-card">
    ...
    <span>${p.professional_area||"Área não informada"}</span>
